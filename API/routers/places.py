...
@router.put("/{id}")
def atualizar_local(id: int, local_data: LocalUpdate, session: SessionDep, current_user: User = Depends(get_current_user)) -> Local:
    local = session.exec(select(Local).where(Local.id == id)).one()
    for key, value in local_data.model_dump(exclude_unset=True).items():
